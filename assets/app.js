...
App = {
  animations: function animations() {
    ...
    if (animationItems.length) {
      var animationItemsObserver = new IntersectionObserver(function (entries) {
      ...
      animationItems.forEach(function (animationItem) {
        return animationItemsObserver.observe(animationItem);
      });
    }
  },
  productShowcase: function productShowcase() {
    var showcaseWraps = document.querySelectorAll('.showcase-wrap');
    if (showcaseWraps.length) {
      var showcaseInit = function showcaseInit() {
        showcaseWraps.forEach(function (showcaseWrap) {
          function init() {
            var wrapWidth = showcaseWrap.parentElement.offsetWidth;
            var wrapHeight = showcaseWrap.parentElement.offsetHeight;
            var articles = showcaseWrap.querySelectorAll('.card');
            if (articles.length) {
              articles.forEach(function (article) {
                ...
                var parent = article.closest('.group');
                var articleWidth = article.offsetWidth;
                var articleHeight = article.offsetHeight;
                var articleLeft = article.offsetLeft + parent.offsetLeft + parent.offsetWidth / 2;
                var articleTop = article.offsetTop + article.closest('.group').offsetTop + parent.offsetHeight / 2;
                var totalW = articleWidth + articleLeft;
                ...
                var totalH = articleHeight + articleTop;
  ...
  headerNav: function headerNav() {
    var details = document.querySelectorAll("header-menu details");
  ...
  navMob: function navMob() {
    var btns = document.querySelectorAll('[data-scrollerbtn]');
    if (btns.length) {
      btns.forEach(function (btn, i) {
        btn.addEventListener('click', function () {
          var scrollAmt = i * -100;
          var scroller = btn.closest('[data-scrollerpar]').querySelector('[data-scroller]');
          var footer = btn.closest('[data-scrollerpar]').querySelector('.footer');
          scroller.style.transform = 'translateX(' + scrollAmt + '%)';
          btn.closest('[data-scrollerpar]').querySelectorAll('[data-scrollerbtn]').forEach(function (sbtn) {
            return sbtn.classList.remove('selected-tab');
          });
          btn.classList.add('selected-tab');
          footer.classList.add('opacity-0');
          setTimeout(function () {
            scroller.querySelectorAll(':scope > li').forEach(function (li) {
    ...
    var openBtn = document.querySelector('[data-navmobopen]');
  ...
  navTrans: function navTrans() {
    var productMobileGallery = document.querySelector('#product-mobile-gallery');
    var productInfoWrap = document.querySelector('#product-info-wrap');
    var prevScroll = 0;
    var scrollUpStart = 0;
    ...
    var headerTransTimer = null;
    var headerTransTimer2 = null;
    var downSetted = false;
    var scrollTop = '';
    var prevScrollEl = '';
    handleScroll = function handleScroll(e) {
      ...
      var offsetHeight = headerBarHeight;
      if (type == 'gallery') {
        offsetHeight = 0;
      }
        ...
        acc.addEventListener('click', function () {
          ...
          if (acc.closest('.acc-parent')) {
            ...
            if (acc.closest('.acc-parent').querySelector('[data-accnext]')) {
              var getTop = acc.closest('.acc-parent').querySelector('[data-accnext]').getBoundingClientRect().top + window.scrollY - document.querySelector('.header-bar').offsetHeight;
              window.scrollTo({
                top: getTop,
                behavior: 'smooth'
              });
  ...
  pageAnchor: function pageAnchor() {
    var pageAnchors = document.querySelectorAll('[data-pageanchor]');
    if (pageAnchors.length) {
      pageAnchors.forEach(function (anchor) {
        var id = anchor.dataset.pageanchor;
        var target = document.getElementById(id);
        if (target) {
          anchor.addEventListener('click', function (e) {
            e.preventDefault();
            var headerBarHeight = document.querySelector('.header-bar').offsetHeight;
            var elTop = target.getBoundingClientRect().top + window.scrollY - headerBarHeight * 1.5;
            window.scrollTo({
              top: elTop,
              behavior: 'smooth'
            });
  ...
  overflowFloatedStyle: function overflowFloatedStyle() {
    var overflowWraps = document.querySelectorAll('.js-overflow-styled');
    if (overflowWraps.length) {
      overflowWraps.forEach(function (overflowWrap) {
        var overflow = overflowWrap.querySelector('.js-overflow-wrap');
        if (overflow) {
          var overflowFloated = overflowWrap.querySelector('.js-overflow-floated');
  ...
  modals: function modals() {
    ...
    var modalCloses = document.querySelectorAll('[data-modalclose]');
    ...
    if (modalTriggers.length) {
      modalTriggers.forEach(function (trigger) {
        var id = trigger.dataset.modaltrigger;
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkchristy"] = self["webpackChunkchristy"] || [];
