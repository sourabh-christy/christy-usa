...
class VariantSelects extends HTMLElement {
    ...
    updateMedia() {
        ...
        const id = this.currentVariant.featured_media.id;
        if (id && mediaGallerys) {
            mediaGallerys.forEach((mediaGallery) => {
                //let first = document.querySelector('#media-34517207056659');
                let move = mediaGallery.querySelector(`#media-${id}`);
                mediaGallery.prepend(move);
            });
    ...
    setUnavailable() {
        const buttons = document.querySelectorAll(`#product-form-${this.dataset.section}`);
        if (!buttons) return;
        ...
        buttons.forEach((button) => {
            ...
            addButtonText = button.querySelector('[name="add"] > span:first-child');
            if (!addButton) return;
